...
(function () {
    ...
    var game = {
        ...
        phaseOrder: [],
        // garde une référence vers l'indice du tableau phaseOrder qu"
        // correspond à la phase de jeu pour le joueur qui commence
        ...
        init: function () {
            ...
            // fonction de choix du joueur qui commence qui
            // sera rappelée lors de la phase de choix
            var chooseWhoStarts = (player1Init, player2Init,
                 player1PLay, player2Play, btnContainer) => {
                ...
                this.level = document.querySelector(
                    ".level-input:checked"
                ).value;
                this.value = (
                    this.value === null
            ...
            if (this.currentPhase === "PHASE_CHOOSE_WHO_STARTS") {
                ...
                document.querySelector(
                 ...
                 () => {
                    chooseWhoStarts(this.PHASE_INIT_PLAYER,
                ...
                document.querySelector(
                    ".choose-computer"
                        ...
                        "click",
                        () => {
                        chooseWhoStarts(this.PHASE_INIT_OPPONENT,
        ...
        setupPlayers: function () {
            // donne à player et computer une réference vers l'objet game
            player.setGame(this);
        ...
        goNextPhase: function () {
            ...
            switch (this.currentPhase) {
                case this.PHASE_GAME_OVER:
                    ...
                    winner = this.gameIsOver();
                    title = document.querySelector("#replay h1");
                    ...
                    if (winner === undefined) {
                        ...
                        document.querySelector(".replay-game")
                            .addEventListener("click", () => {
        ...
        // met le jeu en mode "attente" (les actions joueurs ne doivent pas
        //  être pris en compte si le jeu est dans ce mode)
        wait: function () {
            this.waiting = true;
        },
        ...
        help: function () {
            ...
            cell = this.grid.querySelector(
                `.row:nth-child(${shoot.y + 1}) ` +
                `.cell:nth-child(${shoot.x + 1})`);
        ...
        handleMouseMove: function (e) {
            ...
            if (this.getPhase() === this.PHASE_INIT_PLAYER) {
                if (e.target.classList.contains("cell")) {
                    ...
                    ship = this.players[0].fleet[this.players[0].activeShip];

                    // si on a pas encore affiché (ajouté aux DOM) ce bateau
                    if (!ship.dom.parentNode) {
                        this.grid.appendChild(ship.dom);
                        // passage en arrière plan pour ne pas empêcher la
                        // capture des événements sur les cellules de la grille
                        ship.dom.style.zIndex = -1;
                    }

                    // décalage visuelle, le point d'ancrage du curseurl
                    // est au milieu du bateau

                    // change le décalage si le bateau est pair
                    // et qu'il est vertical
                    halfShip = ship.getLife() / 2;
                    n = Math.floor(halfShip);
                    heightGap = 0;
                    ...
                    ship.dom.style.top = "" +
                        ...
                        heightGap * utils.CELL_SIZE + "px";
                    ship.dom.style.left = "" + utils.eq(e.target) *
                        utils.CELL_SIZE - n * utils.CELL_SIZE + "px";
                }
            }
        },
        handleRightClick: function (e) {
            ...
            if (this.getPhase() === this.PHASE_INIT_PLAYER) {
                if (e.target.classList.contains("cell")) {
                    ship = this.players[0]
                        .fleet[this.players[0].activeShip];
                    ...
                    r = parseInt(ship.dom.style.rotate);
                    r = (
                        r === 0
                        ? 90
                        : 0
        ...
        handleClick: function (e) {
            ...
            if (e.target.classList.contains("cell")) {
                ...
                if (this.getPhase() === this.PHASE_INIT_PLAYER) {
                    // on enregistre la position du bateau, si cela
                    // se passe bien (la fonction renvoie true) on continue
                    if (this.players[0]
                        .setActiveShipPosition(utils.eq(e.target),
                             utils.eq(e.target.parentNode))) {
                    ...
                    if (clickedCell.classList.contains("hit")) {
                        if (clickedCell.classList.contains("miss")) {
                            alreadyClicked = true;
                        }
                    }

                    if(this.grid.querySelector(".suggestion")) {
                        this.grid.querySelector(".suggestion")
                            .classList.remove("suggestion");
                    }

                    // on enregistre le tir
                    this.players[0].play(
                        utils.eq(clickedCell),
                        utils.eq(clickedCell.parentNode),
                        clickedCell, alreadyClicked
                    );
                }
            }
        },
        ...
        fire: function (from, col, line, callback, clickedCell = undefined,
            ...
            // le résultat devra être passé en paramètre à
            // la fonction de callback (3e paramètre)
            target.receiveAttack(col, line, function (hasSucceed) {
                ...
                if (hasSucceed && !alreadyClicked) {
                    ...
                    if (clickedCell !== undefined) {
                        ...
                        cell = self.miniGrid.querySelector(
                            `.row:nth-child(${line + 1}) ` +
        ...
        renderMiniMap: function () {
            ...
            this.miniGrid.style.transform = "scale(0.5) " +
                "translateX(-250px) translateY(-14.25%)";
        }
    };
